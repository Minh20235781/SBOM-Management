...
exports.systemController = {
    ...
    create: async (req, res, next) => {
        try {
            ...
            const existing = await db_1.pool.query('SELECT * FROM system WHERE LOWER(name) = LOWER($1) LIMIT 1', [normalizedName]);
            if (existing.rows.length > 0) {
                const { rows } = await db_1.pool.query(`UPDATE system
           SET description = COALESCE($2, description)
           WHERE system_id = $1
           RETURNING *`, [existing.rows[0].system_id, description || null]);
                return res.json(rows[0]);
            }
            const { rows } = await db_1.pool.query('INSERT INTO system (name, description, last_uploaded_at) VALUES ($1, $2, NULL) RETURNING *', [normalizedName, description || null]);
    ...
    getDetail: async (req, res, next) => {
        ...
        try {
            const systemResult = await db_1.pool.query('SELECT * FROM system WHERE system_id = $1', [id]);
            if (systemResult.rows.length === 0)
                return res.status(404).json({ error: 'System not found' });
            const sboms = await db_1.pool.query(`SELECT
          m.*,
        ...
        WHERE m.system_id = $1
        ORDER BY m.created_timestamp DESC NULLS LAST`, [id]);
            const snapshots = await db_1.pool.query('SELECT * FROM sbom_snapshots WHERE project_id = $1 ORDER BY version_number DESC', [id]);
            const unlinkedSboms = await db_1.pool.query(`SELECT sbom_id, created_timestamp, tool_components, lifecycle_phase
         FROM sbom_metadata
    ...
    linkSbom: async (req, res, next) => {
        const id = Number(req.params.id);
        const { sbomId } = req.body;
        if (!id || Number.isNaN(id))
            return res.status(400).json({ error: 'Invalid id' });
        if (!sbomId)
            return res.status(400).json({ error: 'Missing sbomId' });
        ...
        try {
            ...
            const sbomResult = await client.query('UPDATE sbom_metadata SET system_id = $1 WHERE sbom_id = $2 RETURNING *', [id, sbomId]);
