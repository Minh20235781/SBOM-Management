...
const normalizeRepositoryUrl = (value) => String(value || '')
    .trim()
    .replace(/\.git\/?$/i, '')
    .replace(/\/$/, '')
    .toLowerCase();
const parseGitHubRepository = (repoUrl) => {
    const value = String(repoUrl || '').trim();
    const match = value.match(/^(?:https?:\/\/github\.com\/|git@github\.com:)([^/\s]+)\/([^/\s]+?)(?:\.git)?\/?$/i);
    if (!match)
        throw new Error('INVALID_GITHUB_REPOSITORY: GitHub repository URL is invalid.');
    return { owner: match[1], repo: match[2] };
};
const mapRunStatus = (status, conclusion) => {
    const normalizedStatus = String(status || '').toLowerCase();
    const normalizedConclusion = String(conclusion || '').toLowerCase();
    if (normalizedStatus !== 'completed')
        return normalizedStatus === 'queued' ? 'PENDING' : 'RUNNING';
...
const mapStepStatus = (status, conclusion) => {
    const runStatus = mapRunStatus(status, conclusion);
    return runStatus === 'CANCELLED' ? 'SKIPPED' : runStatus;
};
const nextRunNumber = async (client, pipelineId) => {
    const result = await client.query('SELECT COALESCE(MAX(run_number), 0) + 1 AS run_number FROM cicd_pipeline_runs WHERE pipeline_id = $1', [pipelineId]);
    return Number(result.rows[0].run_number);
};
const findPipeline = async (client, payload) => {
    ...
    if (!repositoryUrl)
        return null;
    const result = await client.query(`SELECT * FROM cicd_pipelines
     WHERE provider = 'GITHUB_ACTIONS'
       AND lower(regexp_replace(repo_url, '\\.git/?$', '')) = $1
     ORDER BY updated_at DESC, pipeline_id DESC
     LIMIT 1`, [repositoryUrl]);
    return result.rows[0] || null;
};
const upsertExternalRun = async (client, pipeline, run) => {
    const externalRunId = String(run?.id || run?.runId || run?.run_id || '');
    if (!externalRunId)
        throw new Error('INVALID_GITHUB_RUN: GitHub run id is required.');
    const existing = await client.query('SELECT * FROM cicd_pipeline_runs WHERE external_run_id = $1 LIMIT 1', [externalRunId]);
    ...
    if (existing.rows[0]) {
        const updated = await client.query(`UPDATE cicd_pipeline_runs SET
        status = $2,
    ...
    const queued = await client.query(`SELECT * FROM cicd_pipeline_runs
     WHERE pipeline_id = $1 AND external_run_id IS NULL AND status = 'PENDING'
     ORDER BY created_at DESC LIMIT 1`, [pipeline.pipeline_id]);
    if (queued.rows[0]) {
        const linked = await client.query(`UPDATE cicd_pipeline_runs SET
       ...
       WHERE run_id = $1 RETURNING *`, [queued.rows[0].run_id, externalRunId, status, run?.head_sha || null,
    ...
    const inserted = await client.query(`INSERT INTO cicd_pipeline_runs
     ...
     RETURNING *`, [pipeline.pipeline_id, pipeline.project_id, runNumber, status,
...
const githubRequest = async (path, init = {}) => {
    ...
    if (!token)
        throw new Error('GITHUB_TOKEN_NOT_CONFIGURED: Configure GITHUB_TOKEN on the backend.');
...
exports.githubActionsService = {
    verifyWebhookSignature: (rawBody, signature) => {
        ...
        if (!secret)
            throw new Error('GITHUB_WEBHOOK_SECRET_NOT_CONFIGURED: Configure GITHUB_WEBHOOK_SECRET.');
        if (!rawBody || !signature)
            return false;
        const expected = `sha256=${crypto_1.default.createHmac('sha256', secret).update(rawBody).digest('hex')}`;
        const left = Buffer.from(expected);
        const right = Buffer.from(signature);
        return left.length === right.length && crypto_1.default.timingSafeEqual(left, right);
    },
    verifyPipelineToken: (authorization) => {
        const expected = process.env.SBOM_PIPELINE_TOKEN;
        if (!expected)
            throw new Error('SBOM_PIPELINE_TOKEN_NOT_CONFIGURED: Configure SBOM_PIPELINE_TOKEN.');
        const actual = authorization?.replace(/^Bearer\s+/i, '') || '';
        const left = Buffer.from(expected);
        const right = Buffer.from(actual);
        return left.length === right.length && crypto_1.default.timingSafeEqual(left, right);
    },
    dispatch: async (client, pipelineId, body = {}) => {
        ...
        if (pipeline.provider !== 'GITHUB_ACTIONS')
            throw new Error('INVALID_PIPELINE_PROVIDER: Pipeline is not configured for GitHub Actions.');
        const { owner, repo } = parseGitHubRepository(pipeline.repo_url);
        const runNumber = await nextRunNumber(client, pipelineId);
        const requestId = crypto_1.default.randomUUID();
        const localRun = await client.query(`INSERT INTO cicd_pipeline_runs
        (pipeline_id, project_id, run_number, status, branch, triggered_by, dispatch_request_id)
       VALUES ($1,$2,$3,'PENDING',$4,$5,$6) RETURNING *`, [pipelineId, pipeline.project_id, runNumber, body?.branch || pipeline.branch,
        ...
        try {
            ...
            await client.query(`UPDATE cicd_pipeline_runs SET status = 'FAILED', conclusion = 'dispatch_failed',
         finished_at = CURRENT_TIMESTAMP WHERE run_id = $1`, [localRun.rows[0].run_id]);
            throw error;
        }
        return { ...localRun.rows[0], dispatch_request_id: requestId, provider: 'GITHUB_ACTIONS' };
    },
    handleWebhook: async (client, event, deliveryId, payload) => {
        if (deliveryId) {
            const inserted = await client.query(`INSERT INTO github_webhook_deliveries (delivery_id, event_name, payload)
         VALUES ($1,$2,$3) ON CONFLICT (delivery_id) DO NOTHING RETURNING delivery_id`, [deliveryId, event, payload]);
            if (!inserted.rows[0])
        ...
        if (event === 'workflow_run') {
            ...
            if (!pipeline)
                return { ignored: true, reason: 'pipeline_not_registered' };
        ...
        if (event === 'workflow_job') {
            ...
            if (steps.length === 0) {
                await client.query(`INSERT INTO cicd_pipeline_steps
           ...
           ON CONFLICT (pipeline_run_id, external_job_id, step_order) DO UPDATE SET
             ...
             finished_at = EXCLUDED.finished_at, logs = EXCLUDED.logs`, [externalRun.run_id, job.name || 'GitHub Actions job', mapStepStatus(job.status, job.conclusion),
             ...
             ON CONFLICT (pipeline_run_id, external_job_id, step_order) DO UPDATE SET
               ...
               logs = EXCLUDED.logs`, [externalRun.run_id, step.name, Number(step.number) || 1,
    ...
    receiveResult: async (client, body) => {
        const pipeline = await findPipeline(client, body);
        if (!pipeline)
            throw new Error('PIPELINE_NOT_FOUND: No GitHub Actions pipeline matches this repository.');
        if (!body?.sbom || typeof body.sbom !== 'object')
            throw new Error('INVALID_SBOM: A CycloneDX or SPDX payload is required.');
        ...
        const sbomId = await (0, sbomParserService_1.parseAndSaveSBOM)(client, { sbom: body.sbom, system_id: pipeline.project_id });
        const snapshot = await incrementalSbomService_1.incrementalSbomService.generate(client, pipeline.project_id, {
            sbom: body.sbom,
            artifactName: `github-actions-${body.runId || body.run_id}-sbom.json`,
            artifactPath: '.github/artifacts/sbom.cdx.json',
        });
        await client.query(`UPDATE cicd_pipeline_runs SET
       ...
       WHERE run_id = $1`, [run.run_id, body.conclusion === 'failure' ? 'FAILED' : 'SUCCESS', body.conclusion || 'success',
    ...
    monitoring: async (client, projectId) => {
        const params = [];
        ...
        const [summary, recent, trend] = await Promise.all([
         ...
         FROM cicd_pipeline_runs r ${where}`, params),
            client.query(`SELECT r.*, p.name AS pipeline_name, p.provider, p.repo_url, p.workflow_file,
         ...
         ${where}
         ORDER BY r.created_at DESC LIMIT 30`, params),
            client.query(`SELECT TO_CHAR(DATE_TRUNC('day', r.created_at), 'YYYY-MM-DD') AS day,
          COUNT(*)::int AS total,
         ...
         ORDER BY DATE_TRUNC('day', r.created_at) DESC LIMIT 14`, params),
        ]);
