...
exports.validationExcelExportService = void 0;
...
exports.validationExcelExportService = {
    generate: async (run) => {
        ...
        addSheet(workbook, 'TongQuan', [{
                ...
                uniqueCve: new Set(findings.map((item) => item.cve_id).filter(Boolean)).size,
                embeddedVulnerabilitiesInInputSbom: analysis.embeddedVulnerabilityCount ?? 0,
                generatedAt: new Date().toISOString(),
            }]);
        const severityCounts = countBySeverity(findings);
        const uniquePackages = new Set(findings.map((item) => `${item.name || ''}@${item.installed || ''}`)).size;
        const fixedAvailable = findings.filter((item) => item.fixed_in).length;
        const epssAvailable = findings.filter((item) => item.epss !== null && item.epss !== undefined).length;
        addSheet(workbook, 'ThongKeCVE', [
            ...
            { metric: 'Unique CVE', value: new Set(findings.map((item) => item.cve_id).filter(Boolean)).size, meaning: 'Distinct CVE identifiers' },
        ...
        const vulnerabilityRows = findings.map((finding) => ({
        ...
        const diffRows = [];
        (verification.MATCHED || []).forEach((component) => diffRows.push({ classification: 'MATCHED', component }));
        (verification.MISSING_IN_SBOM || []).forEach((component) => diffRows.push({ classification: 'MISSING_IN_SBOM', component }));
        (verification.EXTRA_IN_SBOM || []).forEach((component) => diffRows.push({ classification: 'EXTRA_IN_SBOM', component }));
        (verification.VERSION_MISMATCH || []).forEach((item) => diffRows.push({
        ...
        addSheet(workbook, 'TepPhuThuoc', (analysis.dependencyFiles || []).map((file) => ({
        ...
        addSheet(workbook, 'QuyTrinhKiemThu', [
            ...(report.preconditions || []).map((text, index) => ({ type: 'PRECONDITION', order: index + 1, description: text })),
            ...(report.steps || []).map((text, index) => ({ type: 'STEP', order: index + 1, description: text })),
