...
exports.sbomValidationService = void 0;
const artifactScannerService_1 = require("./artifactScannerService");
const displayName = (component) => `${component.name}${component.version ? `@${component.version}` : ''}`;
const mapByKey = (components) => new Map(components.map(component => [component.key, component]));
exports.sbomValidationService = {
    validateSnapshotAgainstSource: async (client, projectId, snapshotId) => {
        const artifacts = await artifactScannerService_1.artifactScannerService.loadProjectArtifacts(client, projectId);
        const scanned = artifactScannerService_1.artifactScannerService.scanArtifacts(projectId, artifacts);
        ...
        const snapshotResult = await client.query(`SELECT stable_key, name, version
       FROM sbom_components
       WHERE snapshot_id = $1
       ORDER BY name`, [snapshotId]);
        ...
        for (const snapshot of snapshotComponents) {
            if (!sourceByKey.has(snapshot.key))
                extraInSbom.push(displayName(snapshot));
        }
