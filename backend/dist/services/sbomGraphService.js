...
exports.sbomGraphService = {
    buildGraph: async (client, snapshotId, options = {}) => {
        const [snapshotResult, componentResult, dependencyResult, vulnerabilityResult] = await Promise.all([
            ...
            client.query(`SELECT c.stable_key, v.severity, v.cve_id, v.vulnerability
        ...
        for (const row of vulnerabilityResult.rows) {
            if (!row.severity && !row.cve_id && !row.vulnerability)
                continue;
        ...
        const depths = new Map([[projectNodeId, 0]]);
        const cycleNodes = new Set();
        const cycleEdges = new Set();
        const queue = [projectNodeId];
        const visitingPath = new Set();
        ...
        let nodes = componentRows.map(row => {
            ...
            const riskLevel = (0, sbomAlgorithms_1.severityToRisk)(vulns.map(v => v.severity));
            return {
                id: row.stable_key,
        ...
        if (options.onlyVulnerable)
            nodes = nodes.filter(node => node.vulnerabilityCount > 0);
        if (search)
            nodes = nodes.filter(node => node.label.toLowerCase().includes(search) || node.purl?.toLowerCase().includes(search));
        ...
        for (const [source, children] of treeChildren) {
            treeChildren.set(source, [...new Set(children)].sort((leftId, rightId) => {
                ...
                return (0, sbomAlgorithms_1.riskRank)(right.riskLevel) - (0, sbomAlgorithms_1.riskRank)(left.riskLevel)
        ...
        const positionNode = (nodeId, depth, path) => {
            const node = nodeById.get(nodeId);
            if (!node)
                return leafIndex * verticalSpacing;
            node.depth = depth;
        ...
        for (const levelNodes of nodesByDepth.values()) {
            ...
            for (const node of levelNodes) {
                if (node.y < nextAvailableY)
                    node.y = nextAvailableY;
        ...
        const countRisk = (risk) => allNodes.filter(node => node.riskLevel === risk).length;
        return {
            snapshotId,
            nodes: allNodes,
            edges,
