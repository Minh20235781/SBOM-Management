...
export const sbomStatusService = {
  forRepository: async (repositoryId: string) => {
    const latestAnalysis = await pool.query(
      `SELECT analysis FROM sbom_validation_runs
    ...
    const { rows } = await pool.query(
      `SELECT m.sbom_id, m.source_commit, m.analyzed_at, r.verification_report
       ...
       ORDER BY m.created_timestamp DESC NULLS LAST LIMIT 1`,
      [repositoryId]
    );
    if (!rows[0]) return {
