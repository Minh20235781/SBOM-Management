...
export type InferredMetadataField = {
  value: string[] | string;
  source: string;
...
export type InferredMetadata = {
  authors: InferredMetadataField;
  services: InferredMetadataField;
  lifecyclePhase: InferredMetadataField & { suggestions?: string[] };
};

const validPhases = new Set(['Plan', 'Code', 'Build', 'Test', 'Release', 'Deploy', 'Operate', 'Monitor']);
const missing = (reason: string): InferredMetadataField => ({
...
const findFiles = async (root: string, names: Set<string>, maxDepth = 4) => {
  ...
  const walk = async (dir: string, depth: number) => {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (!ignored.has(entry.name)) await walk(absolute, depth + 1);
...
const normalizeAuthor = (value: unknown): string | null => {
  ...
  if (typeof value === 'object') {
    const raw = value as { name?: string; email?: string; url?: string };
...
const inferGitAuthors = async (repoPath: string) => {
  try {
...
const inferPackageJson = async (repoPath: string) => {
  const content = await readTextIfExists(path.join(repoPath, 'package.json'));
  if (!content) return { authors: [] as string[], serviceName: null as string | null };
  try {
...
const inferComposerJson = async (repoPath: string) => {
  const content = await readTextIfExists(path.join(repoPath, 'composer.json'));
  if (!content) return { authors: [] as string[], serviceName: null as string | null };
  try {
    const parsed = JSON.parse(content);
    const authors = Array.isArray(parsed.authors) ? parsed.authors.map(normalizeAuthor).filter(Boolean) as string[] : [];
    return {
      authors: unique(authors),
      serviceName: typeof parsed.name === 'string' && parsed.name.trim() ? parsed.name.trim() : null,
    };
...
const inferPomXml = async (repoPath: string) => {
  const content = await readTextIfExists(path.join(repoPath, 'pom.xml'));
  if (!content) return { authors: [] as string[], serviceName: null as string | null };
...
const inferReadmeTitle = async (repoPath: string) => {
...
const inferDockerComposeServices = async (repoPath: string) => {
  ...
  for (const file of files) {
    ...
    if (servicesIndex < 0) continue;
    for (const line of lines.slice(servicesIndex + 1)) {
      if (/^\S/.test(line) && !line.startsWith('services:')) break;
...
const githubOwner = (repoUrl: string) => {
  try {
...
const hasReleaseTag = async (repoPath: string) => {
  try {
    const gitBin = process.env.GIT_BIN || 'git';
...
const hasDeploymentManifests = async (repoPath: string) => {
...
export const metadataInferenceService = {
  infer: async (
    repoPath: string,
    options: { repoUrl: string; repoName: string; context?: 'manual' | 'ci' | 'verify' | 'release' } 
    ...
    const authors = gitAuthors.length > 0
      ? { value: gitAuthors, source: 'git log commit authors', confidence: 'high' as const }
      : fallbackAuthors.length > 0
        ? { value: fallbackAuthors, source: 'package/pom/composer metadata or GitHub owner fallback', confidence: 'medium' as const }
    ...
    const services = composeServices.length > 0
      ? { value: composeServices, source: 'docker-compose services', confidence: 'high' as const }
      : kubernetesServices.length > 0
        ? { value: kubernetesServices, source: 'Kubernetes manifests', confidence: 'high' as const }
        : packageNames.length > 0
          ? { value: packageNames, source: 'package.json/pom.xml/composer.json/README title', confidence: 'medium' as const }
          : options.repoName
            ? { value: [options.repoName], source: 'repository name fallback', confidence: 'low' as const }
    ...
    const lifecyclePhase = {
      ...
      confidence: options.context === 'manual' || !options.context ? 'medium' as const : 'high' as const,
  ...
  injectIntoCycloneDx: (sbom: any, inferred: InferredMetadata) => {
    ...
    const withoutManaged = properties.filter((item: any) => ![
